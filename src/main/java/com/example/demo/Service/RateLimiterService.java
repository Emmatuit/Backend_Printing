...
public class RateLimiterService {
    ...
    public boolean isAllowed(String email, String ip) {
        ...
        RateLimitEntry entry = rateLimitEntryRepository.findByEmailAndIpAddress(email, ip)
                .orElseGet(() -> {
    ...
    public long getRemainingSeconds(String email, String ip) {
        LocalDateTime now = LocalDateTime.now();
        return rateLimitEntryRepository.findByEmailAndIpAddress(email, ip)
                .map(entry -> {
                    long secondsLeft = java.time.Duration.between(now, entry.getLastRequestTime().plusSeconds(COOLDOWN_SECONDS)).getSeconds();
