...
import com.example.demo.Dto.SelectedSpecificationDto;
...
import com.example.demo.model.SelectedSpecification;
...
public class CheckoutService {
	...
	public CheckoutService(CouponService couponService, OrderRepository orderRepository, CartService cartService,
			ProductService productService, UserRepository userRepository, NotificationService notificationService) {
	...
	public OrderDto checkout(CheckoutRequest request, UserEntity user) {
		...
		for (CartItem cartItem : cart.getItems()) {
			...
			List<Long> selectedOptionIds = cartItem.getSelectedOptions().stream().map(option -> option.getId())
			...
			List<SelectedSpecification> selectedSpecifications = new ArrayList<>();
			for (SpecificationOption option : cartItem.getSelectedOptions()) {
				SelectedSpecification selected = new SelectedSpecification();
				selected.setSpecificationName(option.getSpecification().getName()); // or set manually if needed
				selected.setOptionName(option.getName());
				selected.setPrice(option.getPrice());
				selected.setImage(option.getImage());

				selected.setOrderItem(orderItem); // ✅ This fixes the issu

				selectedSpecifications.add(selected);
			}
			orderItem.setSelectedSpecifications(selectedSpecifications);

			// ✅ Copy design request
			if (cartItem.getDesignRequest() != null) {
				DesignRequest design = new DesignRequest();
				design.setFileName(cartItem.getDesignRequest().getFileName());
				design.setFileType(cartItem.getDesignRequest().getFileType());
				design.setFileUrl(cartItem.getDesignRequest().getFileUrl());
				design.setDescription(cartItem.getDesignRequest().getDescription());
				design.setFileId(cartItem.getDesignRequest().getFileId());

				orderItem.setDesignRequest(design);
			}
		...
		BigDecimal vatAmount = (subtotal.subtract(discountAmount).add(deliveryFee)).multiply(vatRate).setScale(2,
		...
		if (request.getShippingMethod() == ShippingMethod.PICKUP) {
			...
			order.setShippingAddress(
					request.getAddress1() + ", " + request.getState() + ", " + request.getPostalCode());
		}
...
//        List<SpecificationOption> selectedOptions = cartItem.getSelectedOptions();
//        orderItem.setSelectedOptions(selectedOptions);
		...
		emailService.sendOrderConfirmationEmail(savedOrder.getEmail(), savedOrder.getFullName(),
				savedOrder.getOrderNumber(), "Your total is ₦" + savedOrder.getGrandTotal());

		// 12. Create admin dashboard notification
		notificationService.createNotification("New Order: " + savedOrder.getOrderNumber(),
				"A new order has been placed by " + user.getUsername() + ". Total: ₦" + savedOrder.getGrandTotal(),
	...
	public OrderDto mapToDto(Order order) {
		...
		dto.setTxRef(order.getTxRef()); // ✅ Added
		dto.setCardLast4(order.getCardLast4()); // ✅ Added
		...
		List<OrderItemDto> items = order.getItems().stream().map(item -> {
			...
			if (item.getProduct() != null) { // ✅ Null-safe mapping
			...
			// ✅ Map selected specifications
			itemDto.setSelectedSpecifications(item.getSelectedSpecifications().stream().map(option -> {
				SelectedSpecificationDto specDto = new SelectedSpecificationDto();
				specDto.setSpecificationName(option.getSpecificationName());
				specDto.setOptionName(option.getOptionName());
				specDto.setPrice(option.getPrice());
				specDto.setImage(option.getImage());
				return specDto;
			}).collect(Collectors.toList()));

			// ✅ Map design request (if present)
			if (item.getDesignRequest() != null) {
				DesignRequestDto drDto = new DesignRequestDto();
				drDto.setFileName(item.getDesignRequest().getFileName());
				drDto.setFileType(item.getDesignRequest().getFileType());
				drDto.setFileUrl(item.getDesignRequest().getFileUrl());
				drDto.setDescription(item.getDesignRequest().getDescription());
				drDto.setFileId(item.getDesignRequest().getFileId());
				itemDto.setDesignRequest(drDto);
			}
	...
	public OrderDto updateOrderStatus(Long orderId, OrderStatus status) {
		...
		// ✅ Generate tracking number if status is now SHIPPED or DELIVERING
